...
- The CDN: R2 bucket `bor-assets` behind `cdn.boxofrules.com`, CORS for the Box Of Rules domains. `tools/publish-cdn.sh` (one rclone sync, seconds) and the `publish-cdn` workflow push every release to `/v<tag>/` (immutable) and `/latest/`. The current working set is on `/latest/` from 19 Sep.
