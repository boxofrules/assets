...
put() { npx --yes wrangler@latest r2 object put "bor-assets/$1/$2" --file "$2" --content-type "$(ctype "$2")" --cache-control "$3" --remote >/dev/null; }
files=$(find css fonts marks web tokens.json manifest.json index.js README.md CHANGELOG.md -type f | sort)
n=0
for f in $files; do
...
echo "published $n files to https://cdn.boxofrules.com/$tag/ and /latest/"
